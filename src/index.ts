...
yargs
  ...
  .command(
    'install-agent', 'Install the backup agent on the server',
    commandYargs =>
      commandYargs.option('y', {
        describe:
          'Disable the questions and use command line args, environment variable and config file',
        boolean: true,
      }),
    installAgent,
  )
  .command(
    ...
    commandYargs =>
      commandYargs
        ...
        .option('daemon-name', { string: true, desc: 'Allows multiple daemons to be started at the same time under different names', }),
    agentLoop,
  )
  .command(
    'restore',
    'Restore a backup from DBacked into a database accessible from this host',
    commandYargs =>
      commandYargs
        ...
        .option('force', { boolean: true, desc: "Don't ask for confirmation before restoring", }),
    restoreBackup,
  )
  .demandCommand()
